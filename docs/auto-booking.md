...
## 아이디·비밀번호로는 안 된다 — 계정 자체가 없다

사용자가 계정 정보를 넘겨서라도 예약을 맡길 수 있느냐고 물었다. 답은 **그런 계정이
없다**는 것이다.

예약 화면에서 읽어낸 입력 요소가 그 증거다.

```
input_spectatorNm, input_spectatorEmail, nation_korean, nation_foreigner,
countrySelect, phone, input_authCode, chk_consent, turnStilecontainer, btn_reserve
```

아이디도 비밀번호도 없다. **예약할 때마다 이름·이메일·국적·휴대폰을 새로 받는다.**
계정이 있는 사이트라면 세션에서 채워 넣지 이렇게 매번 묻지 않는다. 로그인 화면도,
회원가입 경로도 정찰 15차까지 한 번도 나오지 않았다.

즉 오디움에서 "본인"을 증명하는 것은 계정이 아니라 **그 자리에서 받는 문자**다.
그래서 자격 증명을 넘겨받아도 쓸 데가 없다. 넘겨받지 않는 편이 낫기도 하다.

## 그래도 줄일 수 있는 것 — 사람이 쓰는 시간

관문은 못 없애지만 그 앞뒤의 시간은 줄일 수 있고, 취소표 경쟁에서는 그 차이가 크다.

| 줄일 수 있는 것 | 방법 | 효과 |
|---|---|---|
| 감지 지연 | 저장소를 public으로 → Actions 무제한 → 5분 간격 | 최대 10분 → 5분 |
| 이름·이메일·휴대폰 입력 | iOS **설정 → Safari → 자동 완성 → 내 정보** | 세 칸이 한 번에 채워진다 |
| 위가 안 될 때 | 감시 페이지의 "내 예약 정보" 복사 버튼 | 타이핑 없이 붙여넣기 |
| 화면에서 헤매기 | 알림에 담긴 여섯 줄 순서표 | 어디를 누를지 찾을 필요 없음 |

남는 것은 **인증번호 4자리와 체크박스 하나**뿐이다.
